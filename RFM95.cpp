 ...
 */
...
#elif defined(AS_923)
  static const unsigned char LoRa_Frequency[9][3] = {//[923.2 - 924.8] MHz
    { 0xE6, 0xCC, 0xF4 }, //Channel [0], 923.2 MHz / 61.035 Hz = 15125748 = 0xE6CCF4
    { 0xE6, 0xD9, 0xC0 }, //Channel [1], 923.4 MHz / 61.035 Hz = 15129024 = 0xE6D9C0
    { 0xE6, 0xE6, 0x8D }, //Channel [2], 923.6 MHz / 61.035 Hz = 15132301 = 0xE6E68D
    { 0xE6, 0xF3, 0x5A }, //Channel [3], 923.8 MHz / 61.035 Hz = 15135578 = 0xE6F35A
  ...
  static const unsigned char LoRa_Frequency[8][3] = {//[921.4 - 922.8] MHz
...
static unsigned char RFM_Read(unsigned char RFM_Address)
{
  ...
  SpiInOut(RFM_Address & 0x7F);  // Set the register address
  RFM_Data = SpiInOut(0); // Read the register data
...
static void RFM_Change_Channel(unsigned char Channel)
{
  ...
  if (Channel <= 0x08)
    for(unsigned char i = 0 ; i < 3 ; ++i)
      RFM_Write(RFM_REG_FR_MSB + i, LoRa_Frequency[Channel][i]);
  else if (Channel == 0x10)
    for(unsigned char i = 0 ; i < 3 ; ++i)
      RFM_Write(RFM_REG_FR_MSB + i, LoRa_Frequency[0][i]);
#elif defined(EU_868)
  ...
  if (Channel <= 0x07)
    for(unsigned char i = 0 ; i < 3 ; ++i)
      RFM_Write(RFM_REG_FR_MSB + i, LoRa_TX_Freq[Channel][i]);
  else if (Channel >= 0x08 && Channel <= 0x0F)
    for(unsigned char i = 0 ; i < 3 ; ++i) { 
      RFM_Write(RFM_REG_FR_MSB + i, LoRa_RX_Freq[Channel - 0x08][i]);
    }
#endif
}
...
void RFM_Write(unsigned char RFM_Address, unsigned char RFM_Data)
{
  ...
  SpiInOut(RFM_Address | 0x80 );  // Send register address and Set MSB for 'write' command
  SpiInOut(RFM_Data);             // Send the data to write
